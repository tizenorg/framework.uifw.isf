      ...
      m_old_x_error_handler (0)
{
    ...
    m_panel_client.signal_connect_select_aux                    (slot (this, &X11FrontEnd::panel_slot_select_aux));
    m_panel_client.signal_connect_show_preedit_string           (slot (this, &X11FrontEnd::panel_slot_show_preedit_string));
    m_panel_client.signal_connect_hide_preedit_string           (slot (this, &X11FrontEnd::panel_slot_hide_preedit_string));
    m_panel_client.signal_connect_update_preedit_string         (slot (this, &X11FrontEnd::panel_slot_update_preedit_string));
}
...
X11FrontEnd::filter_hotkeys (X11IC *ic, const KeyEvent &scimkey)
{
    ...
    if (hotkey_action == SCIM_FRONTEND_HOTKEY_TRIGGER) {
        ...
        String encoding = scim_get_locale_encoding (ic->locale);
        String language = scim_get_locale_language (ic->locale);
        ISEInfo info = m_imengine_hotkey_matcher.get_match_result ();
        String  sfid = info.uuid;
        if (info.type == IMENGINE_T) {
            if (validate_factory (sfid, encoding)) {
                ims_turn_off_ic (ic);
                replace_instance (ic->siid, sfid);
                m_panel_client.register_input_context (ic->icid, get_instance_uuid (ic->siid));
...
X11FrontEnd::ims_create_ic_handler (XIMS ims, IMChangeICStruct *call_data)
{
    ...
    if (m_shared_input_method) {
        ...
        String sfid = get_default_factory (language, encoding);
        siid = new_instance (m_config, sfid, encoding);
    }
...
X11FrontEnd::ims_set_ic_focus_handler (XIMS ims, IMChangeFocusStruct *call_data)
{
    ...
    if (m_shared_input_method) {
        ...
        String sfid = get_default_factory (language, encoding);
        ic->siid = new_instance (m_config, sfid, encoding);
        ic->onspot_preedit_started = false;
        ic->onspot_preedit_length = 0;
        ic->onspot_caret = 0;
...
void
X11FrontEnd::panel_slot_select_aux (int context, int aux_index)
{
    X11IC *ic = m_ic_manager.find_ic (context);
    SCIM_DEBUG_FRONTEND(1) << __FUNCTION__ << " context=" << context << " aux=" << aux_index << " ic=" << ic << "\n";
    if (validate_ic (ic)) {
        m_panel_client.prepare (ic->icid);
        select_aux (ic->siid,aux_index);
        m_panel_client.send ();
    }
}

void
X11FrontEnd::panel_slot_show_preedit_string (int context)
{
    X11IC *ic = m_ic_manager.find_ic (context);
    SCIM_DEBUG_FRONTEND(1) << __FUNCTION__ << " context=" << context << "\n";
    if (validate_ic (ic)) {
        show_preedit_string(ic->siid);
    }
}

void
X11FrontEnd::panel_slot_hide_preedit_string (int context)
{
    SCIM_DEBUG_FRONTEND(1) << __FUNCTION__ << "...\n";
    X11IC *ic = m_ic_manager.find_ic (context);
    if (validate_ic (ic)) {
        hide_preedit_string(ic->siid);
    }
}
    ...
    if (target &&
        ...
        if (m_focus_ic->pre_attr.spot_location.x >= 0 &&
            ...
            XTranslateCoordinates (m_display, target,
                ...
                xwa.height,
                &spot_x, &spot_y, &child);
            spot_top_y = spot_y;  // Fix me
        }
        m_panel_client.update_spot_location (ic->icid, spot_x, spot_y, spot_top_y);
    }
}
